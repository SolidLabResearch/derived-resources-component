import { App, AppRunner, joinFilePath, joinUrl } from '@solid/community-server';
...
describe('The server test setup', (): void => {
  ...
  beforeAll(async(): Promise<void> => {
    app = await new AppRunner().create(
      {
        config: [
          joinFilePath(__dirname, '../../config/example-index.json'),
  ...
  it('returns the derived index resource.', async(): Promise<void> => {
    const res = await fetch(joinUrl(baseUrl, 'type'));
    const store = await responseToStore(res, joinUrl(baseUrl, 'type'));
    ...
    expect(store.countQuads(subjects[0], DERIVED_INDEX.terms.instance, namedNode(joinUrl(baseUrl, 'data/public')), null)).toBe(1);
  });

  it('returns more resources when authorized.', async(): Promise<void> => {
    const res = await fetch(joinUrl(baseUrl, 'type'), { headers: { Authorization: 'WebID http://example.com/alice' } });
    const store = await responseToStore(res, joinUrl(baseUrl, 'type'));
    const subjects = store.getSubjects(DERIVED_INDEX.terms.for, namedNode('http://xmlns.com/foaf/0.1/Agent'), null);
    expect(subjects).toHaveLength(1);
    expect(store.countQuads(subjects[0], DERIVED_INDEX.terms.instance, null, null)).toBe(2);
    expect(store.countQuads(subjects[0], DERIVED_INDEX.terms.instance, namedNode(joinUrl(baseUrl, 'data/public')), null)).toBe(1);
    expect(store.countQuads(subjects[0], DERIVED_INDEX.terms.instance, namedNode(joinUrl(baseUrl, 'data/alice')), null)).toBe(1);
  });
});
